...
`risk-corpus.js`、`expert-corpus.js` 與網站一起部署，目前包含：

- 1,871 筆逐筆高風險用語／近義詞／常見變體。
- 1,349 筆繁體中文完整案例相似語料。
- 11 組既有語句／上下文結構規則，另加 1.6.0 語言學結構規則。
- 17 組既有工作內容合理性規則，另加高風險工作語言學阻擋規則。
- 17 組來源索引。
...
輸出風格分為自然工作訊息、精簡直接及正式書面。1.6.0 另加入目的導向語序規劃與內容保留檢核，並在 UI 允許直接切換三個版本比較。這是可審計的規則式語言生成，目的在於比固定模板更自然；不宣稱等同大型語言模型。
